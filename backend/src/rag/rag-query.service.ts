...
export class RagQueryService {
  ...
  async query(
    ...
    if (motorId) {
      ...
      if (!ctx) {
        return {
          answer: `Motor ${motorId} not found in the system.`,
          sources: [],
          warnings: [],
        };
      }
      ...
      if (disconnected.length > 0) {
        for (const s of disconnected) {
          warnings.push(
            `No recent data available for ${s.sensorType} sensor (status: ${s.status}).`,
          );
        }
      }
      ...
      if (disconnected.length === ctx.sensors.length) {
        return {
          answer: `I don't have recent readings from motor ${motorId}. All sensors are currently disconnected or without data. Please check the Grafana dashboard for historical information.`,
          sources: [],
          warnings,
        };
      }
    ...
    if (this.isHistoricalQuestion(question) && !knowledgeResults.length) {
      return {
        answer: `For historical data and trends I recommend checking the Grafana dashboard, which has access to aggregated historical readings. I can only answer about the current state of the plant.`,
        sources: [],
        warnings,
      };
    }
    ...
    if (!liveContextBlock && knowledgeResults.length === 0) {
      return {
        answer: `I don't have sufficient information to answer this question. If you're asking about a specific motor, please include the motor_id. For general maintenance knowledge, try rephrasing your question.`,
        sources: [],
        warnings,
      };
    }
    ...
    if (faultSensors.length > 0) {
      warnings.push(
        `Sensors in fault state (data unreliable): ${faultSensors.join(', ')}.`,
      );
    }
  ...
  private buildSystemPrompt(faultSensors: string[]): string {
    let prompt = `You are an industrial IoT assistant for a plant with 15 motors, each with 3 sensors (temperature, vibration, current).

RULES (non-negotiable):
1. Only state facts you have data for. If a sensor has no recent reading, say so explicitly.
2. If a sensor is in fault/stuck/fault_persistent state, its value is UNRELIABLE. Never cite it as a verified fact.
3. For deep historical questions (last week, last month, trends over time), redirect to Grafana.
4. If you don't have enough information, say so clearly. Never invent causes or values.
5. Keep answers concise and actionable for an operator.
6. When citing thresholds or standards, mention the source (ISO 10816-3, NEMA MG-1, etc.).`;

    if (faultSensors.length > 0) {
      prompt += `\n\nCRITICAL: The following sensors are currently in FAULT state — their values are UNRELIABLE and must NOT be cited as facts: ${faultSensors.join(', ')}.`;
    }
  ...
  private async callGroq(
    ...
    if (!this.groqApiKey) {
      ...
      return 'The AI assistant is not configured (missing API key). Please contact the administrator.';
    }
    ...
    try {
      ...
      if (!response.ok) {
        ...
        return 'I was unable to process your question at this time. Please try again later.';
      }
